...
count = spark.table("nessie.ecommerce.product_recommendations_ml").count()
print(f"Total Rows in product_recommendations_ml: {count:}")
spark.sql("SELECT * FROM nessie.ecommerce.product_recommendations_ml LIMIT 5").show()
spark.stop()
