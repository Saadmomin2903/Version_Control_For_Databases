"""
REAL ML Pipeline - "Full Power" Version (FIXED)
-----------------------------------------------
- Uses 100% of Data (No Sampling).
- Fixes 'VectorUDT' error by dropping vector columns before saving.
- Optimizes memory by persisting only aggregated customer profiles.
"""
...
print("=" * 60)
print("🚀 REAL ML PIPELINE - FULL DATA (FIXED)")
print("=" * 60)
print("⚠️  Processing 300M+ Rows. Expect 1-2 Hours Execution Time.")
print("=" * 60)

spark = (SparkSession.builder
    .appName("real-ml-pipeline-fixed")
    ...
    .config("spark.sql.shuffle.partitions", "300") # Increased for 300M rows
    .config("spark.driver.memory", "6g")
    .config("spark.executor.memory", "6g")
    .config("spark.memory.fraction", "0.8")        # Dedicate more to execution
    .getOrCreate())
...
try:
    ...
    # 1. Transactions (Purchases only) - Reduced columns for memory
    transactions_df = spark.sql("""
        SELECT customer_id, product_id, price, order_date, category_code
    ...
    # FPGrowth works on customer baskets, likely much smaller than 300M
    ...
    # ============================================
    print("\n🛠️  Engineering Advanced Features (Aggregating 300M rows)...")
    
    # Get Max Date for Recency
    max_date_row = transactions_df.agg(F.max("order_date")).collect()
    ...
    # Trends (Simplified for Performance on 300M)
    ...
    # Views/Carts (Aggregated separately to avoid huge join explosion before agg)
    events_agg = spark.sql("""
        SELECT customer_id, 
    ...
    # Combine Features - This results in 1 row per customer (manageable size)
    full_features = base_features \
        .join(trends_enriched, "customer_id", "left") \
        .join(events_agg, "customer_id", "left") \
        .na.fill(0)
    
    # PERSIST AGGREGATED FEATURES ONLY (Crucial for performance)
    full_features.cache()
    count = full_features.count()
    print(f"   Engineered features for {count:,} unique customers")
    ...
    # FIX: Drop vector columns before writing
    segments_clean = segments.drop("features_raw", "features")
    segments_clean.writeTo("nessie.ecommerce.customer_segments_ml").createOrReplace()
    print("   ✅ customer_segments_ml updated")
    
    # ============================================
    # STAGE 3: Churn Prediction
    # ============================================
    print("\n📌 STAGE 3/5: Churn Prediction")
    
    churn_data = segments.withColumn("churn_label", F.when(F.col("recency_days") > 90, 1.0).otherwise(0.0))
    churn_cols = ["recency_days", "frequency", "monetary_value", "customer_lifetime_days", "revenue_trend", "total_views"]
    ...
    gbt = GBTClassifier(labelCol="churn_label", featuresCol="churn_features", maxIter=20) # Redux iter for speed
    ...
    final_churn = predictions.withColumn("churn_probability", extract_prob("probability")) \
        .select("customer_id", "churn_probability", "recency_days")
    ...
    # ============================================
    print("\n📌 STAGE 4/5: CLV Prediction")
    
    clv_cols = ["recency_days", "frequency", "avg_order_value", "customer_lifetime_days", "revenue_last_90d", "revenue_trend"]
    ...
    gbt = GBTRegressor(labelCol="monetary_value", featuresCol="clv_features", maxIter=20)
    
    pipeline = Pipeline(stages=[assembler, scaler, gbt])
    model = pipeline.fit(full_features)
    predictions = model.transform(full_features)
    
    final_clv = predictions.select("customer_id", F.col("prediction").alias("predicted_clv_12m"))
    ...
    # ============================================
    print("\n📌 STAGE 5/5: Next Purchase")
    
    w_lag = Window.partitionBy("customer_id").orderBy("order_date")
    interval_stats = transactions_df.select("customer_id", "order_date").distinct() \
        ...
        .groupBy("customer_id").agg(F.avg("days_between").alias("avg_interval"))
    
    npp_data = full_features.join(interval_stats, "customer_id", "left").na.fill(0)
    
    npp_cols = ["recency_days", "frequency", "avg_interval", "avg_order_value"]
    ...
    gbt = GBTRegressor(labelCol="avg_interval", featuresCol="npp_features", maxIter=20)
    
    pipeline = Pipeline(stages=[assembler, scaler, gbt])
    model = pipeline.fit(npp_data)
    predictions = model.transform(npp_data)
    
    final_npp = predictions.select("customer_id", F.col("prediction").alias("predicted_interval"))
    ...
    print("\n🎉 Verification Completed. All 300M rows processed.")
