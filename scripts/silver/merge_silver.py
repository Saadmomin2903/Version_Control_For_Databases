...
try:
    # Check if tables exist and have data in main
    o_count = spark.sql("SELECT COUNT(*) as cnt FROM nessie.ecommerce.orders_silver").collect()[0]['cnt']
    print(f"   [Orders]    Row Check: {o_count:,} records found in nessie.ecommerce.orders_silver (on main)")
    
    c_count = spark.sql("SELECT COUNT(*) as cnt FROM nessie.ecommerce.customers_silver").collect()[0]['cnt']
    print(f"   [Customers] Row Check: {c_count:,} records found in nessie.ecommerce.customers_silver (on main)")
